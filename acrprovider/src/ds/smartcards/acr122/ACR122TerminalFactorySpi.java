...
public class ACR122TerminalFactorySpi extends TerminalFactorySpi {
    ...
    /** The maximum size of the block sent out to the card. Could probably be larger. */
    private static final int TR_BLOCK_SIZE = 32;
    
    /** The maximum timeout for the card to reply to an APDU. Set to 2 seconds. 
      * This could probably set to infinite, but it may happen that the card session 
      * will lock (I have seen this happening). If the card needs more than 2 seconds
      * to process an APDU it has to explicitly ask for a time extension (I assume),
      *  see below in ACRChannel.transmit.
      */
    ...
    /** Constants and commands found in the ACR122 documentation: 
      ...
      * Some (RAW_SEND) are not documented there. 
      * More generally these are commands for the PN532 controller. 
      */
    ...
    private static final byte[] POLL_COMMAND = new byte[] { ACR_BYTE, 0x4A, 0x01, 0x00 };
    ...
    private class ACR122CardTerminals extends CardTerminals {
        ...
        public List<CardTerminal> list(State state) throws CardException {
            if (terminal.virtualCard != null) {
                ...
                if (state == CardTerminals.State.CARD_PRESENT
                        && terminal.state == PRESENT) {
                    return list();
                }else
                if (state == CardTerminals.State.CARD_ABSENT
        ...
        /** Wait for a change (card insertion or removal) in any of the terminals. 
    ...
    private class ACR122CardTerminal extends CardTerminal {
        ...
        /** Whether a card has been inserted or removed since the last call to waitFor...() */
        private boolean inserted = false;
        private boolean removed = false;

        /** The ACR virtual card (ATR 3B00) and channel we will be talking to
         * to set the protocol parameters and to communicate with the actual card.  
         */
        ...
        /** The sequence byte that is used in the ISO14443 protocol. Alternates between 0x0A and 0x0B.
         * (Actuall it is a one bit the byte that alternates, but the rest of the byte never changes
         * in our case.
         */
        ...
        /** Construct the terminal. First finds the actual PC/SC ACR terminal through the
         * regular Sun PCSC provider. Then initializes a connection to the virtual card and
         * sets some preliminary protocol paramers (reset the antenna, get the reader id and
         * store it, set timeout to zero (see ACR122 API docs), and switch off the automatic 
        ...
        /** Connect to the card. For now ignore the protocol parameter, only store. */
        ...
        public boolean isCardPresent() throws CardException {
            ...
            synchronized(channel) {
                ...
                if(res[3] == pByte && res[4] == CID) {
                    ...
                    // Poll the card
                    res = channel.transmit(new CommandAPDU(ACRcommand(POLL_COMMAND))).getBytes();
                    ...
                    if (res[index] == (byte) 0x00) {
                        ...
                        // The card is there, get its UID and ATS/ATR, set the communication speed.
                        state = PRESENT;
                        sequenceByte = (byte)0x0A;
                        index += 3;
        ...
        /** Wait for a given card presence state (present or absent) with a timeout. Also note whether the new state
         * (after the required state has been reached) is different from the previous one. 
         * 
         * @param timeout the timeout
         * @param present whether we wait for presence or absence
    ...
    private class ACRCard extends Card {
        ...
        /** The virtual card channel we will be talking to to communicate with the actual card. */
        private CardChannel virtualChannel = null;

        private ACRCard(byte[] ats, ACR122CardTerminal terminal, String protocol,
                CardChannel channel) {
            super();
            atr = new ATR(ats);
        ...
        public ATR getATR() {
            if (terminal.state == ABSENT)
                throw new IllegalStateException("Card is not present or disconnected.");
            return atr;
        }

        /** Gains exclusive access to the card. NB. The ACR reader can probably talk to more than one
         * card at the same time. I am not sure the smardcardio API allows this. 
         */ 
        ...
        public void disconnect(boolean reset) throws CardException {
            if (terminal.state == ABSENT)
                throw new IllegalStateException("Card is not present or already disconnected.");
        ...
        /** Builds the basic logical channel (0) to communicate with the application on the card.
         */
        public CardChannel getBasicChannel() {
            ...
            try {
                ...
                // This comment is right. When the ACRChannel constructor is called with
        ...
        /** Opens a new logical channel (1+) to communicate with the application on the card.
         */
        ...
        /** Transmits a control command to the card. For now we assume that we transmit a raw
         * data to the ACR virtual card.
         */
    ...
    /** This is where the actual communication with the card using the
     * ISO14443-4 protocol happens. This is a bit messy :D */
    private class ACRChannel extends CardChannel {
        ...
        /** Construct a new channel. For channels > 0 (non-basic) use the MANAGE CHANNEL
         * command to open a new channel.
         */
        ...
        /** This method is used to set the terminal state to "card absent". It
         * is called by all the methods that may suspect that the card may have been removed, due to
         * e.g. certain communication problems.
         * 
         * @param setState should the terminal state be set
         * @throws CardException
         */
        ...
        /** Wraps a portion of an APDU into a ACR send APDU command. Parameter last indicates if
         * this is the last portion, in which case the chaining bit is set to 0. 
         */
        ...
        /** Unwraps a portion of the response APDU sent back by the ACR terminal. Checks the wrapping bytes for
         * correctness of the protocol.
         */
        private byte[] unwrapAPDU(byte[] r) throws CardException {
            debug("unwrapAPDU 1: "+byteArrayToString(r, false));
            if (r[0] != ACR_BYTE_ACK || r[1] != RAW_SEND_ACK
                    || r[2] != RESP_BYTE_OK || (byte)(r[3] & (byte)0x0F) != card.terminal.sequenceByte || r[4] != CID) {
                noCard(true);
            }
        ...
        public ResponseAPDU transmit(CommandAPDU cmd) throws CardException {
            ...
            synchronized(card.virtualChannel) {
                ...
                // First send all the blocks minus the last one with the chaining bit set, check the
                ...
                // Here we are at the point where we start receiving the response APDU. 
                // If the card is not ready with the response we give it TIMEOUT milliseconds
                // to come up with one. It is also at this point that the card may wish to 
                // request a processing time extension. In this case we restart counting the timeout.                  
                long start = System.currentTimeMillis();
                CommandAPDU c = new CommandAPDU(ACRcommand(card.terminal.getPing()));
                CommandAPDU cext = null;
                while(resp.getBytes()[2] != RESP_BYTE_OK || (byte)(resp.getBytes()[3] & 0xF0) == (byte)0xF0) {
                ...
                // The answer should be ready, get it from terminal piece by piece, acknowlege every piece. 
                byte[] r = null;
        ...
        /** Transmit an APDU placed in the cmd buffer, put the response in the resp buffer. 
