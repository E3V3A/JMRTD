...
public class PassportPersoService extends PassportApduService {
   ...
   private void putPrivateKey(SecureMessagingWrapper wrapper,
         byte[] privModulus, byte[] privExponent) throws CardServiceException {
      try {
   ...
   public CommandAPDU createUpdateBinaryAPDU(short offset, int data_len, byte[] data) {
      ...
      // System.out.println("DEBUG: offset = " + offset + " data.length = " + data.length + " chunk.length = " + chunk.length + " data_len = " + data_len);
   ...
   public byte[] sendUpdateBinary(SecureMessagingWrapper wrapper,
         short offset, int data_len, byte[] data) throws CardServiceException {
   ...
   public void writeFile(SecureMessagingWrapper wrapper, short fid,
         InputStream i) throws CardServiceException {
      try {
         int length = 0xff;
         if (wrapper != null) {
            length -= 32;
   ...
   public void writeMRZ(byte[] docNr, byte[] dob, byte[] doe) throws CardServiceException {
   ...
   public void lockApplet(SecureMessagingWrapper wrapper) throws CardServiceException {
